...
def read_param(cn, bcn, x):
    ...
    print(row)
    data = df.at[row, "buy_frac"]
    return data

def Reb(coin, base_coin, pair_min = 0, pair_max = 0, div = 0):
    ...
    coin_dist = float(read_param(coin, base_coin, 'coin_dist'))
    print(coin_dist)
...
def rebalance():
    ...
    while trade_count == 0:
        ...
        '''
        if trade_count == 0:
            trade_count = trade_count+Reb('FTM', 'USDT', 2, 0.86)
        if trade_count == 0:
            trade_count = trade_count+Reb('SEI', 'USDT', 2, 0.9)
        if trade_count == 0:
            trade_count = trade_count+Reb('NEO', 'USDT', 2, 0.85)
        if trade_count == 0:
            trade_count = trade_count+Reb('FIL', 'USDT', 2, 0.86)
        if trade_count == 0:
            trade_count = trade_count+Reb('LINK', 'USDT', 2, 0.89)
        if trade_count == 0:
            trade_count = trade_count+Reb('AVAX', 'USDT', 2, 0.92)
        if trade_count == 0:
            trade_count = trade_count+Reb('MATIC', 'USDT', 2, 0.93)
        if trade_count == 0:
            trade_count = trade_count+Reb('ADA', 'USDT', 2, 0.91)
        if trade_count == 0:
            trade_count = trade_count+Reb('PAXG', 'USDT', 2, 0.65)
        if trade_count == 0:
            trade_count = trade_count+Reb('XRP', 'USDT', 2, 0.9)
        if trade_count == 0:
            trade_count = trade_count+Reb('ETH', 'BTC', 5, 0, 0.00826, 0.11902, 0.9)
        if trade_count == 0:
            trade_count = trade_count+Reb('SOL', 'BTC', 9, 0, 0.00006, 0.00372, 1.27)
        '''
    return trade_count
