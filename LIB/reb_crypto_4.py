...
#from tradingview_indicator_test import *
from cbbi_indicator_test import *
...
def reb(coin_pair):
    ...
    try:
        if crypto_adj < 0 and trade_percent >= sell_frac:
            ...
            print('\n' + str(crypto_trade) + ' ' + coin + '-' + coin_pair + ' sold')
            count = count+1
            ...
            print('\n' + str(crypto_trade) + ' ' + coin + '-' + coin_pair + ' bought')
            count = count+1
...
def print_status():
    ...
    for i in lst:
       ...
       #print(i, crypto)
        if i != 'USDC':
            crypto_USDT = float(read_price(i+'USDC'))
