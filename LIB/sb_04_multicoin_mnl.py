...
def Reb(coin, base_coin, buy_frac, sell_frac, coin_dist=0, pair_min = 0, pair_max = 0, div = 0):
    ...
    elif coin_dist == 0 and pair_min != 0 and pair_max != 0 and div != 0:
...
def rebalance():
    ...
    while trade_count == 0:
        if trade_count == 0:
            trade_count = trade_count+Reb('BNB', 'BTC', 20, 22, 0.01)
        if trade_count == 0:
            trade_count = trade_count+Reb('NEO', 'BTC', 10, 12, 0.03)
        if trade_count == 0:
            trade_count = trade_count+Reb('FTM', 'BTC', 10, 12, 0.03)
        if trade_count == 0:
            trade_count = trade_count+Reb('FIL', 'BTC', 10, 12, 0.03)
        if trade_count == 0:
            trade_count = trade_count+Reb('LINK', 'BTC', 10, 12, 0.05)
        if trade_count == 0:
            trade_count = trade_count+Reb('SEI', 'BTC', 10, 10, 0.04)
        if trade_count == 0:
            trade_count = trade_count+Reb('XRP', 'BTC', 10, 20, 0.05)
        if trade_count == 0:
            trade_count = trade_count+Reb('ADA', 'BTC', 9, 10, 0.13)
        if trade_count == 0:
            trade_count = trade_count+Reb('AVAX', 'BTC', 8, 15, 0.08)
        if trade_count == 0:
            trade_count = trade_count+Reb('MATIC', 'BTC', 8, 12, 0.06)
        if trade_count == 0:
            trade_count = trade_count+Reb('SOL', 'BTC', 9, 9, 0, 0.00006, 0.00372, 1.17)
        if trade_count == 0:
            trade_count = trade_count+Reb('ETH', 'BTC', 3.8, 5, 0, 0.00826, 0.11902, 0.9)
        if trade_count == 0:
            trade_count = trade_count+Reb('LINK', 'BTC', 17, 25, 0, 0.0000324, 0.0005575, 10)
        if trade_count == 0:
            if float(read_price('BTCUSDT')) <= 40000:
                div_pax = 8
            elif float(read_price('BTCUSDT')) <= 39000:
                div_pax = 9
            elif float(read_price('BTCUSDT')) <= 38000:
                div_pax = 10
            elif float(read_price('BTCUSDT')) <= 37000:
                div_pax = 15
            elif float(read_price('BTCUSDT')) <= 36000:
                div_pax = 20
            elif float(read_price('BTCUSDT')) <= 35000:
                div_pax = 70
            else:
                div_pax = 6
            trade_count = trade_count+Reb('PAXG', 'BTC', 10, 3, 0, 0.02833, 0.25278, div_pax)
            if float(read_price('BTCUSDT')) <= 35000:
                exit()
    return trade_count
